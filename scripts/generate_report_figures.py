...
def plot_soil_distributions(soil_df: pd.DataFrame, output_dir: str):
    ...
    if all(c in soil_df.columns for c in ['sand', 'silt', 'clay']):
        ...
        axes[1, 0].bar(['Sand', 'Silt', 'Clay'], avg_texture, color=['yellow', 'gray', 'brown'], alpha=0.7)
    ...
    if 'cec' in soil_df.columns and 'clay' in soil_df.columns:
        ...
        p = np.poly1d(z)
        axes[1, 2].plot(soil_df['clay'].sort_values(), p(soil_df['clay'].sort_values()), 
                       'r--', linewidth=2, label=f'r={soil_df["clay"].corr(soil_df["cec"]):.2f}')
        axes[1, 2].legend()
...
def plot_cdl_analysis(cdl_df: pd.DataFrame, output_dir: str):
    ...
    Parameters:
        ...
        output_dir: Output directory for figures
    """
    year_cols = [c for c in cdl_df.columns if 'cdl_' in c.lower() or 
                 (isinstance(c, str) and c.startswith('20') and len(c) == 4)]
...
def generate_all_figures(weather_csv: str = None, soil_csv: str = None, 
                         cdl_csv: str = None, output_dir: str = 'docs/project/reports/figures'):
    ...
    if weather_csv:
        ...
        if 'GDD' not in weather_df.columns:
            weather_df['GDD'] = ((weather_df['T2M_MAX'].clip(upper=30) + 
                                  weather_df['T2M_MIN'].clip(lower=10)) / 2) - 10
