...
def field_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    field_agg.columns = [
        "_".join(col).strip() if isinstance(col, tuple) else col for col in field_agg.columns
    ]
...
def drainage_analysis(df: pd.DataFrame) -> pd.DataFrame:
    ...
    drainage_summary = (
        df.groupby("drainage_class")
        .agg({"ph": ["mean", "std"], "om": ["mean", "std"], "cec": "mean", "awc": "mean"})
        .round(2)
    )
...
def analyze_soil(csv_path: str, output_dir: str = None) -> dict:
    ...
    results = {"summary": {}, "correlations": {}, "by_field": {}, "by_drainage": None}
...
if __name__ == "__main__":
    ...
    for prop, stats_dict in results["summary"].items():
        if isinstance(stats_dict, dict):
            ...
            print(
                f"  Range: {stats_dict.get('min', 'N/A'):.2f} - {stats_dict.get('max', 'N/A'):.2f}"
            )
